class AcquireGame {
    ...
    awardBonus(chainLetter) {
        ...
        if (topPlayers.length > 1 || secondPlayers.length === 0) {
            ...
            if (secondPlayers.length === 0 && topPlayers.length === 1) {
                report += `Player ${topPlayers[0]} is the solo holder, (${top} shares) $${split} for both majority and minority.\n`
            } else {
    ...
    stepThrough(inputval) {
        ...
        if (this.playerPhase === this.stockRequestPhase || this.playerPhase === this.noStockPhase) {
            const retval = this.stockRequest(inputval)
            // don't drop through if we're moving on to the next turn
            if (retval.length > 2 || this.clientserver === "client") {
                return retval
            }
        }
        ...
        this.secretMessage = this.textBack.concat(this.board()).concat('Player ',String(this.playerTurn),`, you are holding\n\nMoney $${String(this.playerFunds[this.playerTurn])}\n\n${this.playerStockHoldings(false)}${removedMessage}`,
                                     "Choose a tile to play:\n\n",this.showPlayerTiles(), gameEndMessage)
        if (this.clientserver === "client") {
            return this.secretMessage
        }
        return this.textBack.concat(this.board(), 'Player ', String(this.playerTurn), "'s turn")
    }

} // end of acquire function

if (typeof module !== 'undefined' && module.exports) module.exports = { AcquireGame }
